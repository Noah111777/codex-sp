      ...
      _onSelectChange: function() {
        ...
        .then(cart => {
          for (var i = 0; i < cart.items.length; i++) {
            if (cart.items[i].id == variant.id) {
              cartQTY = cart.items[i].quantity; 
            }
          }    
          inputOBJ.setAttribute('data-cartqty',cartQTY); // SET cart qty   
          var availQTY = maxQTY - cartQTY;
          // Set QTY VAL and ATC
          inputOBJ.value = availQTY ? 1 : 0; // Reset quantity selector
  ...
  theme.CartForm = (function() {
    ...
    CartForm.prototype = Object.assign({}, CartForm.prototype, {
      ...
      quantityChanged: function(evt) {
        ...
        console.log("ST:QTYCHANGED"); // no code
  ...
  theme.QtySelector = (function() {
    ...
    QtySelector.prototype = Object.assign({}, QtySelector.prototype, {
      init: function() {
        ...
        console.log("ST:QTYSELINIT");
        if (this.options.isCart) {
          console.log(""); // no code
        } else {
          var varID = this.input.getAttribute('data-varid');
          // Get MAX QTY
          var maxQTY = this.input.getAttribute('max');
          // Get CART QTY
          var cartQTY = 0;
          var url = ''.concat(theme.routes.cart, '?t=').concat(Date.now());
          var cartContents = fetch(url, {
          ...
          .then(cart => {
            ...
            this.input.setAttribute('data-cartqty',cartQTY); // SET cart qty   
            var availQTY = maxQTY - cartQTY;
            // Set QTY VAL and ATC
            this.input.value = availQTY ? 1 : 0; 
            var cartBtn = document.querySelector('.add-to-cart');
        ...
        document.addEventListener('ajaxProduct:added', function(evt) {
          ...
          // [ST] On Product Add when ATC is clicked            
            if (evt.detail != null) { // atc btn clicked
              ...
              .then(cart => {
                for (var i = 0; i < cart.items.length; i++) {
                  if (cart.items[i].id == evt.detail.product.id) {
                    cartQTY = cart.items[i].quantity; 
                  }
                }
          ...
          console.log("Ajax Product added");
          //let currentPath = window.location.pathname;
          //let sourceBtn = localStorage.getItem('quickadd');

          // if (currentPath.includes("/collections") && !currentPath.includes("/products") && evt.type == "ajaxProduct:added" && sourceBtn == 'true')  {
          //   console.log("Don't open cart drawer");
            if (!config.cartOpen) {
              document.querySelector('.site-header__drawer').classList.add('hide-cart');
              document.querySelector('.site-nav__close-cart').classList.add('hide-cart');
              document.querySelector('.site-nav__icons').classList.add('hide-cart');
              
              this.open();
              setTimeout(function() {
                // document.querySelector('.site-header__drawer').style.display = 'block';
                // document.querySelector('.site-nav__icons').style.display = 'unset';
                // document.querySelector('.site-nav__icons').style.display = 'flex';
                this.close();
                document.querySelector('.site-header__drawer').classList.remove('hide-cart');
                document.querySelector('.site-header__drawer').classList.remove('is-transitioning');
                document.querySelector('.site-nav__close-cart').classList.remove('hide-cart');
                document.querySelector('.site-nav__icons').classList.remove('hide-cart');
              }.bind(this), 1000);
            }
          // }
          // else {
    ...
    QuickAdd.prototype = Object.assign({}, QuickAdd.prototype, {
      ...
      loadQuickAddForm: function(evt) {
        ...
        fetch(url).then(function(response) {
          ...
          console.log("ST:MODALOPN");
          /*           
          var var_quantity = 0;

          var varCont = this.quickAddHolder.querySelector('.product-single__variants');
          var varID = varCont.options[varCont.selectedIndex].value;
          this.quickAddHolder.querySelectorAll('.js-variant-inventory-data').forEach(el => {
            var invCont = el.getAttribute('data-id');
            if (invCont == varID) {              
              var_quantity = el.dataset.quantity;
              // var_policy = el.dataset.policy;
              // var_incoming = el.dataset.incoming;
              // var_next_incoming_date = el.dataset.date;
            }
          });
          var maxqty = var_quantity; // MAX QTY
          ...
          .then(cart => {
            for (var i = 0; i < cart.items.length; i++) {
              if (cart.items[i].id == varID) {
                cartqty = cart.items[i].quantity; // CART QTY
              }
            }          
            var prodQtyObj = document.querySelector(".prodqty");
            if (prodQtyObj != null) {

              prodQtyObj.setAttribute('max',maxqty); // SET max qty to stock level
              prodQtyObj.setAttribute('data-cartqty',cartqty); // SET cart qty
              var availqty = maxqty - cartqty;
              prodQtyObj.value = availqty ? 1 : 0; // Reset quantity selector
              var cartBtn = document.querySelector('.add-to-cart');
              if (prodQtyObj.value == 0) {
                cartBtn.classList.add("disabled");
                cartBtn.disabled = true;
              } else {
                cartBtn.classList.remove("disabled");
      ...
      updateInventory: function(evt) {
        ...
        if (variant.inventory_management === 'shopify' && window.inventories && window.inventories[this.productId]) {
          ...
          .then(cart => {
            ...
            // Reset quantity selector in product forms
            var prodQtyObj = document.querySelector(".prodqty");
            if (prodQtyObj != null) {
              prodQtyObj.setAttribute('max',quantity); // SET max qty to stock level
              prodQtyObj.setAttribute('data-cartqty',cartqty); // SET cart qty based on current cart value 
              var availQty = quantity - cartqty;
              prodQtyObj.value = availQty ? 1 : 0; // Reset quantity selector
