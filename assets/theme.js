...
(function(){
  ...
  theme.Variants = (function() {
    ...
    Variants.prototype = Object.assign({}, Variants.prototype, {
      ...
      _onSelectChange: function() {
        ...
        // [ST] On Variant Change
        console.log("ST:VARCHG");
        var inputOBJ = document.querySelector(".prodqty");
        inputOBJ.setAttribute('data-varid',variant.id); // SET max qty
        // Set MAXQTY
        var maxQTY = inputOBJ.getAttribute('max');
        this.container.querySelectorAll('.js-variant-inventory-data').forEach(el => {
          var invCont = el.getAttribute('data-id');
          if (invCont == variant.id) {
            if (el.dataset.policy == 'deny') {
              maxQTY = el.dataset.quantity;
            }
            else {
              maxQTY = '9999999';
            }
            // var_policy = el.dataset.policy;
            // var_incoming = el.dataset.incoming;
            // var_next_incoming_date = el.dataset.date;
          }
        });
        inputOBJ.setAttribute('max',maxQTY); // SET max qty 
        // Call Quantity Limiter
        var status = updateQuantityLimit();
        // [ST] End
      },
    ...
    CartForm.prototype = Object.assign({}, CartForm.prototype, {
      init: function() {
        ...
        if (this.noteInput) {
          ...
          this.noteInput.addEventListener('change', saveNote);
          this.noteInput.addEventListener('blur', saveNote);
        }
      ...
      cartMarkup: function(html) {
        ...
        document.dispatchEvent(new CustomEvent('cart:rebuild'))
      },
  ...
  theme.AjaxProduct = (function() {
    ...
    ProductForm.prototype = Object.assign({}, ProductForm.prototype, {
      addItemFromForm: function(evt, callback){
        ...
        // console.log("adding item from form");
        // localStorage.setItem('quickadd','true');
      ...
      success: function(product) {
        var prodQtyObj = document.querySelector(".prodqty");
        if (prodQtyObj != null) {
          prodQtyObj.value = "1"; // REVISED: Reset quantity selector to 1
        }
        ...
        // let sourceBtn = localStorage.getItem('quickadd');  
        // if (sourceBtn != 'truse') {
        ...
        // console.log("Added item from quickadd / quickshop");
  ...
  theme.QtySelector = (function() {
    ...
    function QtySelector(el, options) {
      ...
      this.cartQty = this.input.getAttribute('data-cartqty'); 
    ...
    QtySelector.prototype = Object.assign({}, QtySelector.prototype, {
      init: function() {
        ...
        // [ST] Initialize the Quantity Selector
      ...
      _change: function(qty) {
        // [ST] On Quantity Select Change 
        console.log("ST:QTYSEL");
        this.maxValue = this.input.getAttribute('max'); 
        this.cartQty = this.input.getAttribute('data-cartqty'); 
        if (this.options.isCart) { 
          this.maxValue = this.maxValue;
        } else {
          this.maxValue = (this.maxValue - this.cartQty);
        }
        // [ST] End
        ...
        if (this.options.isCart) {
          ...
          // [ST] On Cart Qty Select Change:      
          //console.log("ST:CARTCHG"); 
          var cartID = this.input.getAttribute('data-varid');
          var prodObj = document.querySelector(".prodqty");
          if (prodObj != null) {
            var varID = prodObj.getAttribute('data-varid');            
            if (varID == cartID) {
              prodObj.setAttribute('data-cartqty',qty); 
              var done = updateATCBtn();
              /*
              var maxQTY = prodObj.getAttribute('max');
              var availQTY = maxQTY - qty;
              prodObj.value = availQTY ? 1:0; 
    ...
    function buildProducts(results) {
      ...
      results.forEach(product => {
        $.ajax({
          ...
          dataType: 'json',
          async: false,
          success: function(hc_product) {
            var new_product = {
              title: product.title,
              url: product.url,
              id: hc_product.id,
              variants: hc_product.variants,
              tags: hc_product.tags,
    ...
    HeaderCart.prototype = Object.assign({}, HeaderCart.prototype, {
      init: function() {
        ...
        document.addEventListener('ajaxProduct:added', function(evt) {
          ...
          // [ST] On Product Add when ATC is clicked            
            if (evt.detail != null) { // atc btn clicked
              //console.log("ST:PRODADD");
              // Call Quantity Limiter
              var status = updateQuantityLimit();
            }
          // [ST] End

         /* HC - Drawer fix - 30 Mar '24 */ 
         if (!config.cartOpen) {
            this.open();
          }
          // }
          ...
          // let sourceBtn = localStorage.getItem('quickadd');
          // if (currentPath.includes("/collections") && !currentPath.includes("/products") && evt.type == "ajaxProduct:added" && sourceBtn == 'true')  {
      ...
      close: function(evt) {
        ...
        var noteInput = this.wrapper.querySelector('[name="note"]');
        if (noteInput && noteInput.value) {
          theme.cart.updateNote(noteInput.value);
        }
  ...
  theme.QuickAdd = (function() {
    ...
    QuickAdd.prototype = Object.assign({}, QuickAdd.prototype, {
      init: function() {
        ...
        /* [st] always open modal for quickadd 
        if (quickAddBtns) {
        ...
        /* [st] even with single variant, modal should popup */
  ...
  theme.buildProductGridItem = function(items, imageSize) {
    ...
    items.forEach(product => {
      ...
      var hcAccountTags = document.querySelector('#HeaderWrapper').getAttribute('data-customer');
      var hcProductTags = product.tags;
      var hcHTML = '';
      
      let logintoseetag = false;
      let balmainprod = false;
      let kmprod = false;
      let euforaprod = false;
      let azprod = false;
      let resthaircolor = false;
      let restprairie = false;
      let restbc = false;

      hcProductTags.forEach(function (hcTag, i) {
        let ptag = hcTag.toLowerCase();
        if (ptag.includes('logintosee')) logintoseetag = true;
        if (ptag.includes('fbrand:balmain')) balmainprod = true;
        if (ptag.includes('fbrand:km')) kmprod = true;
        if (ptag.includes('fbrand:eufora')) euforaprod = true;
        if (ptag.includes('restrict:az')) azprod = true;
        if (ptag.includes('restrict:hair color')) resthaircolor = true;
        if (ptag.includes('restrict:prairieonly')) restprairie = true;
        if (ptag.includes('restrict:bconly')) restbc = true;
      });

      let customerlogin = false;
      let balmaincust = false;
      let kmcust = false;
      let euforacust = false;
      let azcust = false;
      let hairsalon = false;
      let hairstybar = false;
      let regprairie = false;
      let regbc = false;
      let blink = false;

      if (hcAccountTags != '') customerlogin = true;

      let ctag = hcAccountTags.toLowerCase();
      if (ctag.indexOf('brand:balmain') != -1) balmaincust = true;
      if (ctag.indexOf('brand:km') != -1) kmcust = true;
      if (ctag.indexOf('brand:eufora') != -1) euforacust = true;
      if (ctag.indexOf('brand:az') != -1) azcust = true;
      if (ctag.indexOf('hair salon') != -1) hairsalon = true;
      if (ctag.indexOf('hairsty/barber') != -1) hairstybar = true;
      if (ctag.indexOf('region:prairie') != -1) regprairie = true;
      if (ctag.indexOf('region:bc')) regbc = true;
      if (ctag.indexOf('bluelinktagged') != -1) blink = true;

      let hidelogin = false;
      let hidebalmain = false;
      let hidekm = false;
      let hideeufora = false;
      let hideaz = false;
      let hidehair = false;
      let hideprairie = false;
      let hidebc = false;

      if (logintoseetag) {
        if (customerlogin) {
            hidelogin = false;
            if (balmainprod && !balmaincust) hidebalmain = true;
            if (kmprod && !kmcust) hidekm = true;
            if (euforaprod && !euforacust) hideeufora = true;
            if (azprod && !azcust) hideaz = true;
            if (resthaircolor) hidehair = !(hairsalon || hairstybar);
            if (restprairie && !regprairie) hideprairie = true;
            if (restbc && !regbc) hidebc = true;
        } else {
            hidelogin = true;
        }
      }
    
      let showblinkmsg = false;
      if (logintoseetag && customerlogin && !blink) showblinkmsg = true;
      
      let hcShowATC = true;
      let hidetags = false;

      if (hidebalmain || hidekm || hideeufora || hideaz || hidehair) hidetags = true;
      if (hidelogin || hidetags || hideprairie || hidebc || showblinkmsg) hcShowATC = false;

      if(hcShowATC){
        hcHTML = '<div class="hc-atc-form">';
        if(product.variants.length > 1){
          hcHTML += '<select class="hc-select" data-product="'+product.id+'">'; 
          product.variants.forEach(function (hcVariant, i) {
            hcHTML += '<option value="'+hcVariant.id+'">"'+hcVariant.title+'"</option>';
          });
          hcHTML += '</select>'; 
        } 
        hcHTML += '<div class="hc-add-form">';
        hcHTML += '<div class="hc-quantity js-qty__wrapper"><button type="button" value="-" data-product="'+product.id+'" class="qtyminus js-qty__adjust js-qty__adjust--minus" field="quantity"><svg aria-hidden="true" focusable="false" role="presentation" class="icon icon-minus" viewBox="0 0 64 64"><path class="cls-1" d="M55 32H9"/></svg><span class="icon__fallback-text" aria-hidden="true">&minus;</span></button>';
        hcHTML += '<input type="text" data-field="quantity" data-product="'+product.id+'" hidden="hidden" name="quantity" value="1" min="1" class="js-quantity-selector js-qty__num prodqty">';
        hcHTML += '<button type="button" value="+" data-product="'+product.id+'" class="qtyplus js-qty__adjust js-qty__adjust--plus" field="quantity"><svg aria-hidden="true" focusable="false" role="presentation" class="icon icon-plus" viewBox="0 0 64 64"><path class="cls-1" d="M32 9v46m23-23H9"/></svg><span class="icon__fallback-text" aria-hidden="true">+</span></button></div>';
        hcHTML += '<button class="hc-atc btn btn--small" data-qty="1" data-product="'+product.id+'" data-id="'+product.variants[0].id+'"><span class="hc-atc-text">Add to Cart</span><span class="dot-container"><span class="hc-dot"></span><span class="hc-dot"></span><span class="hc-dot"></span></span></button></div></div>';
      }
  ...
  theme.buildProductImage = function(product, imageSize) {
    ...
    var hcImageSearch = product.image_responsive_url;

    if(hcImageSearch == undefined){
      hcImageSearch = hcImage;
    }
        ...
        if (this.sortBtns.length) {
          this.sortBtns.forEach(btn => {
            btn.addEventListener('click', function() {
              ...
              this.queryParams.set('sort_by', btn.dataset.value);
  ...
  theme.Product = (function() {
    ...
    Product.prototype = Object.assign({}, Product.prototype, {
      ...
      addIdToRecentlyViewed: function() {
        ...
        // Limit to 50 items
        if (theme.recentlyViewedIds.length > 50) {
          theme.recentlyViewedIds = theme.recentlyViewedIds.slice(0, 50);
        }
      ...
      updatePrice: function(evt) {
        ...
        if (variant) {
          ...
          let bssLtspElements = target.querySelectorAll(".bsscommerce-ltsp-message");
          
            
          if (bssLtspElements.length > 0) return;
  ...
  // let ltspMessageCheckTimes = 0;
  // let ltspCheck = setInterval(function() {
  //               let productRecommend = document.querySelector('.product-recommendations .grid-product__price');
  //               if (productRecommend || ltspMessageCheckTimes > 100) {
  //                   document.querySelectorAll('.grid-product__price').forEach(function(e) {
  //                     let ltspMessage = e.querySelectorAll('.bsscommerce-ltsp-message');
  //                     if (ltspMessage.length > 1) {
  //                         e.querySelector('.bsscommerce-ltsp-message a').remove();
  //                         e.querySelector('.grid-product__price--savings').remove();
  //                     }
  //                   })
  //                   clearInterval(ltspCheck);
  //               }
  //               ltspMessageCheckTimes += 1;
  //           },300)
